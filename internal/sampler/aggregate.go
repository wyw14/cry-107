...
func (a *Aggregator) Complete(batchID uuid.UUID) (BatchSamples, bool) {
	a.mu.RLock()
	defer a.mu.RUnlock()
	batch := BatchSamples{BatchID: batchID}
	for _, candidate := range a.batches {
		if candidate.Gas != nil {
			batch.Gas = candidate.Gas
		}
		if candidate.Dust != nil {
			batch.Dust = candidate.Dust
		}
	}
	return batch, batch.Gas != nil && batch.Dust != nil
}
